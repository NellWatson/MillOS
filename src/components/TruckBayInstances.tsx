...
const useTranslatedGeometry = (GeometryClass: any, args: any[], translation: [number, number, number], rotation?: [number, number, number]) => {
    return useMemo(() => {
...
export const OptimizedTrafficConeInstances: React.FC<{
    ...
    const stripe1Geo = useTranslatedGeometry(THREE.CylinderGeometry, [0.09, 0.11, 0.08, 8], [0, 0.2, 0]);
    const stripe2Geo = useTranslatedGeometry(THREE.CylinderGeometry, [0.06, 0.08, 0.06, 8], [0, 0.35, 0]);
...
export const OptimizedSpeedBumpInstances: React.FC<{
    ...
    const data = useMemo(() => bumps.map(b => ({
        position: b.position,
...
export const OptimizedStripeInstances: React.FC<{
    ...
    color?: string;
}> = React.memo(({ positions, rotation = [-Math.PI / 2, 0, 0] as [number, number, number], color = "#fef3c7" }) => {
